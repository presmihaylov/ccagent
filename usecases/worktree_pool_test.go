...
func TestAcquire_EmptyPool(t *testing.T) {
	...
	_, err := pool.Acquire("job-123", "ccagent/test-branch")
	if err == nil {
...
func TestAcquire_FromPool(t *testing.T) {
	...
	// Acquire a worktree
	wtPath, err := pool.Acquire("job-test-123", "ccagent/test-feature")
	if err != nil {
	...
	if branchName != "ccagent/test-feature" {
		t.Errorf("Expected branch 'ccagent/test-feature', got '%s'", branchName)
	}
...
func TestConcurrentAcquire(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		...
		go func(idx int) {
			...
			branchName := "ccagent/concurrent-" + string(rune('a'+idx))
...
func TestReplenish_AfterAcquire(t *testing.T) {
	...
	// Acquire one
	_, acquireErr := pool.Acquire("job-replenish-test", "ccagent/replenish-test")
	if acquireErr != nil {
...
func TestReclaimOrphanedPoolWorktrees(t *testing.T) {
	...
	branchName := "ccagent/pool-ready-orphan123"
